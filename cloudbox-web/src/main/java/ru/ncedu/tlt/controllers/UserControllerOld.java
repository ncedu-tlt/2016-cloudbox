...
public class UserControllerOld {
    ...
    private static volatile UserControllerOld instance;

    //Временные поля
    HashMap<String, User> userList;
    private int num = 0;
    
    private UserControllerOld() {
    ...
    public static UserControllerOld getInstance() {
        UserControllerOld localInstance = instance;
        if (localInstance == null) {
            synchronized (UserControllerOld.class) {
                localInstance = instance;
                if (localInstance == null) {
                    instance = localInstance = new UserControllerOld();
                }
            }
        }
